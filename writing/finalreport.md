...
By using A* algorithm, the maze path finding game was created. In this program, we have use some code from [Red Blob Games](https://www.redblobgames.com/pathfinding/a-star/implementation.html). Basically, we used ```argparse``` to manage the input and output files. User have to manually design their map in a ```txt``` file. In the source directory, there are two python programs. ```a_star``` is the main program and implementation.py contains all the necessary classes. As the chart shows:

![Program functions](float.png)

The ```get_matrix``` function gets the information of designed map, then other functions will use these information to calculate the shortest path and draw it in the output file. The ```heuristic``` function calculate the Manhattan distance between two nodes. ```a_star``` function will use this value with classes, such as ```PriorityQueue``` and ```SquareGrid``` to get the path information. Finally, ```draw_tile``` and ```draw_grid``` functions would be used to draw the path.

As the screenshot shows:

![Demo](output.png)

Out result contains the user input and the solved shortest path.

Detailed information on how to run the program can be found [here](../src/readme.md)
